...
const crearSolicitud = async (req, res) => {
  try {
    ...
    const publicacionDB = await pool.query(
      `
      SELECT *
      FROM publicaciones
      WHERE id = $1
    ...
    if (publicacionDB.rows.length === 0) {
  ...
  try {
    ...
    const solicitudes = await pool.query(`
      SELECT
        ...
        solicitudes_compra.cantidad_solicitada,
        solicitudes_compra.estado,
        solicitudes_compra.created_at
